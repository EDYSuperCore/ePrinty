      ...
      "printers": [
        {
          ...
          "driver_path": "C:\\path\\to\\driver.inf",
          "driver_install_policy": "always"
        },
        {
          "name": "大厦A座一楼 前台",
          "path": "\\\\192.168.20.11",
...
- `areas`: 办公区数组
  ...
  - `printers`: 该办公区的打印机列表
    - `name`: 打印机名称（必需）
    - `path`: 打印机网络路径（必需，Windows 格式：`\\IP地址`）
    - `model`: 打印机型号（可选，会在 UI 中显示）
    - `driver_path`: 驱动路径（可选，用于自定义驱动安装）
    - `driver_install_policy`: 驱动安装策略（可选，`"always"` 或 `"reuse_if_installed"`）
